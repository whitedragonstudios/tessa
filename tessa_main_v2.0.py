...
def load_config():
    ...
    try: 
        ...
        config_file = {"hdd" : "/", "city":"Error Loading", "country":"", "longitude":0.0, "latitude":0.0, "source":"Error Loading", "source list": ["Error", "Loading"], "denied list": [""], "news key":"", "weather key":"", "updated":"", "forecast_error" : [{"key":"value"}]}
...
def f2_forecast_report(weather_json):
    ...
    try:
        ...
        forecast_report = ['Connection Error', 'Connection Error', 'Connection Error', 'Connection Error', 'Connection Error', 'Connection Error', 'Connection Error', 'Connection Error']
...
config_handle = load_config()
hdd = config_handle['hdd']
city = config_handle['city']
country = config_handle['country']
lon = config_handle["longitude"]
lat = config_handle["latitude"]
# forecast_error = config_handle["forecast_error"]
weather_key = config_handle['weather key']
...
# list of pages for the switch function to referance
...
def refresher():
    ...
    f6_storage_report()
    f7_change_source_buttons()
    root.after(18000, refresher) # 30 minute cycle 1800000
...
# inciment the decimal value. Alpha will be after v1.7.
...
# ajdusted get news to be passed as param so it can be used for...
# ...F4 and F5 
# normalized fonts
...
# added global assignement config load and function calls within settings functions
...
# 2.0
# Version split 'main' shall be exclusively designed for smart watch functionality.
# "PC" will be exclusively designed for a computer interface.
# Removed next/back buttons >>> not needed for main
# added error catching if HDD is not detected <def storage_report>
# added harddrive value to config file
# fixed ERROR when not connected to the internet the program hangs. Added error messages piped to Weather, Forecast, Healines, and News GUI displays.
